...
/*   Updated: 2019/08/01 15:50:38 by fbabin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int			pprint(t_env *env)
{
	...
	while (env->text_size >= 16)
	{
		...
		pflush(env, "\t", 1);
		if (print_oline(env, (char*)((size_t)env->text_raddr + offset), 16, env->mod) == -1)
			return (-1);
		pflush(env, "\n", 1);
		offset += 16;
		env->text_size -= 16;
	...
	if (env->text_size > 0)
	{
		print_address(env, env->text_addr + offset, 8);
		pflush(env, "\t", 1);
		if (print_oline(env, (char*)((size_t)env->text_raddr + offset), env->text_size, env->mod) == -1)
			return (-1);
		pflush(env, "\n", 1);
	}
